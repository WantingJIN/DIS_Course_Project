...
float migr[2] = {3, -1.67};				// Migration vector
char *robot_name;
...
void process_received_weightings_from_supervisor()
{
	...
	if (wb_receiver_get_queue_length(receiver_radio) > 0)
	{
		...
		// rule2_weight = rbuffer[1];
		// rule3_weight = rbuffer[2];
		// migration_weight = rbuffer[3];
...
int main()
{
	...
	for (;;)
	{
		...
		for (t = 0; t < loop_num; t++)
		{
			...
			msl += bmsl;
			msr += bmsr;
			//printf("Add Braitenberg msl is: %d, msr is: %d\n", msl, msr);
			// Set speed
